...
const Hero: React.FC = () => {
  ...
  return (
    <section className="relative min-h-[90vh] flex items-center overflow-hidden bg-gradient-to-br from-primary-900 via-primary-800 to-contrast-900">
      {/* Background Pattern */}
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className={`space-y-8 ${isRTL ? 'lg:order-2 text-right' : ''}`}>
            <div className="space-y-6">
              ...
              <p className="text-xl lg:text-2xl text-secondary-200 leading-relaxed max-w-2xl">
                {t('heroSubtitle')}
              </p>
            </div>
            ...
            <div className={`flex flex-col sm:flex-row gap-4 ${isRTL ? 'sm:flex-row-reverse' : ''}`}>
              ...
              <Link
                href={`/${language}/about`}
                className="border-2 border-secondary-300 hover:border-highlight-500 text-white hover:text-highlight-500 px-8 py-4 rounded-xl font-semibold transition-all duration-300 inline-flex items-center justify-center gap-3 backdrop-blur-sm hover:bg-white/10"
            ...
            <div className="flex items-center gap-4 pt-4">
              <div className="flex text-highlight-400 text-lg">
                {'★'.repeat(5)}
              </div>
          ...
          <div className={`${isRTL ? 'lg:order-1' : ''} relative`}>
            <div className="relative group">
              {/* Main Card */}
              <div className="relative bg-gradient-to-br from-secondary-100 to-base-200 rounded-2xl p-8 lg:p-12 shadow-2xl transform group-hover:scale-105 transition-all duration-500">
                <div className="text-center">
                  <div className="text-8xl lg:text-9xl mb-6 transform group-hover:rotate-6 transition-transform duration-500">
                    🛏️
                  </div>
                  <h3 className="text-2xl lg:text-3xl font-bold text-contrast-800 mb-2">
                    {t('premiumBedding')}
                  </h3>
              ...
              {/* Decorative Elements */}
              <div className="absolute -top-4 -left-4 w-8 h-8 bg-neutral-400 rounded-full opacity-80 animate-pulse" />
              <div className="absolute top-1/4 -right-2 w-6 h-6 bg-highlight-400 rounded-full opacity-60 animate-pulse delay-300" />
              <div className="absolute -bottom-2 left-1/4 w-4 h-4 bg-primary-300 rounded-full opacity-70 animate-pulse delay-700" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 animate-bounce">
        <div className="w-6 h-10 border-2 border-secondary-300 rounded-full flex justify-center">
          <div className="w-1 h-3 bg-secondary-300 rounded-full mt-2 animate-pulse" />
        </div>
      </div>
    </section>
  );
