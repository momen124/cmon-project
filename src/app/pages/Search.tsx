...
const Search: React.FC = () => {
  ...
  const highlightText = (text: string, query: string) => {
    ...
    return parts.map((part, index) =>
      regex.test(part) ? (
        <mark key={index} className="bg-accent-200 px-1 rounded font-english">
          {part}
        </mark>
      ) : (
        part
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-primary-50 animate-slide-up">
      <div className="mb-8">
        ...
        <form onSubmit={handleSearch} className="mb-6">
          <div className="relative max-w-2xl">
            <input
              ...
              className={`w-full px-4 py-3 border border-neutral-200 rounded-lg bg-primary-50 text-text-primary-600 focus:outline-none focus:ring-2 focus:ring-accent-500 font-english ${isRTL ? 'pr-12' : 'pl-12'}`}
            />
            <MagnifyingGlassIcon className={`absolute top-3.5 w-6 h-6 text-neutral-600 ${isRTL ? 'right-3' : 'left-3'}`} />
            <button
              type="submit"
              className={`absolute top-2 bottom-2 px-6 bg-text-primary-600 text-primary-50 rounded-lg hover:bg-accent-500 hover:text-text-primary-600 transition-colors hover-lift font-english ${isRTL ? 'left-2' : 'right-2'}`}
            >
              {t('search')}
            </button>
          </div>
        ...
        {searchQuery && (
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between bg-primary-50 rounded-lg shadow-sm border border-neutral-200 p-4">
            <div>
      ...
      {!searchQuery ? (
        <div className="text-center py-16">
          <MagnifyingGlassIcon className="w-24 h-24 mx-auto text-neutral-600" />
          <h2 className="text-2xl font-semibold text-text-primary-600 mb-4 font-english">{t('searchProducts')}</h2>
          <p className="text-neutral-600 mb-8 max-w-md mx-auto font-english">
            {isRTL 
          ...
          <div className="max-w-2xl mx-auto">
            <h3 className="text-lg font-semibold text-text-primary-600 mb-4 font-english">{t('popularSearches')}</h3>
            <div className={`flex flex-wrap gap-2 justify-center ${isRTL ? 'space-x-reverse' : ''}`}>
              {['bed sheets', 'duvet', 'towels', 'cotton', 'pillows', 'queen size'].map((term) => (
                <button
                  ...
                  className="bg-neutral-100 hover:bg-accent-200 hover:text-text-primary-600 px-4 py-2 rounded-full text-sm transition-colors hover-lift font-english"
                >
                  {term}
                </button>
              ))}
        ...
        <div className="text-center py-16">
          <AdjustmentsHorizontalIcon className="w-24 h-24 mx-auto text-neutral-600" />
          <h2 className="text-2xl font-semibold text-text-primary-600 mb-4 font-english">{t('noResultsFound')}</h2>
          <p className="text-neutral-600 mb-8 max-w-md mx-auto font-english">
            {isRTL 
          ...
          <div className="max-w-2xl mx-auto">
            <h3 className="text-lg font-semibold text-text-primary-600 mb-4 font-english">{t('searchSuggestions')}</h3>
            <ul className="text-sm text-neutral-600 space-y-1 font-english">
